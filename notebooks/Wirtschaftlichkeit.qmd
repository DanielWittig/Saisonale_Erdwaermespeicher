...
## merge old dna
old_dna = pd.read_csv('workbook_dna.csv')
dedup = dedup.merge(old_dna, on='sheetcell', how='left')
dedup = dedup.iloc[:, [0,1,4]]
dedup.columns = ['sheetcell', 'step', 'sheetformula']
...
new_for_dedup.loc[:, 'influencer_cell'] = new_for_dedup.loc[:, 'influencer_cell'].str.replace(r'\$', '', regex=True)
new_for_dedup.loc[:, 'formula'] = new_for_dedup.loc[:, 'formula'].str.replace(r'\$', '', regex=True)
...
#unique
new_for_dedup = new_for_dedup.drop_duplicates('sheetcell')

#new dna
new_dna = pure_dedup.merge(
  new_for_dedup.loc[:,['sheetcell', 'sheetformula']]
...
old_dna = old_dna.loc[:,['sheetcell', 'step', 'sheetformula']]

# save dna
old_dna.to_csv('workbook_dna.csv')
...
mask_new =\
  ...
  .isin(dedup.loc[mask_history,'sheetcell'])
...
o put sheetformulas into dedup
o generate variable names - based based on rules (where efficient) from excel
